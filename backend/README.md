...
FastAPI 기반 백엔드입니다. 현재 백엔드는 정책 조회 API와 소상공인24 주기 크롤러를 포함합니다.
...
Docker crawler service 소상공인24 주기 크롤링
```
...
크롤러는 소상공인24 통합공고 목록에서 아래 조건을 사용합니다.
...
`soboksobok_db`가 실행 중이고 `0.0.0.0:5431->5432/tcp` 포트 매핑이 보여야 합니다. 테이블은 FastAPI 서버 시작 시 `Base.metadata.create_all(bind=engine)`로 생성됩니다. 수집 데이터가 비어 있으면 아래 명령으로 크롤러를 한 번 실행합니다.
