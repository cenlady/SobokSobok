...
const STAGES = [
  ...
  { key: 'diagnose', label: '서류 내용 확인 중', statuses: ['diagnosing'] },
] as const
  ...
  return (
    <div className="pb-6">
      ...
      <section className="px-5 pt-6">
        <h2 className="page-title">서류 검토</h2>
        <p className="mt-1.5 text-sm leading-relaxed text-muted">
          제출 전에 오타와 빈칸을 점검하고 정책에서 요구하는 서류를 확인합니다.
        </p>
      </section>
    ...
    <>
      <section className="mt-6 px-5">
        <h3 className="text-sm font-semibold text-brand-dark">
          <span className="mr-2 font-bold tabular-nums text-brand">01</span> 어떤 정책에 낼 서류인가요?
        </h3>
        ...
        {noSaved ? (
          <div className="surface-panel mt-3 p-4">
            ...
            <button
              onClick={onGoFind}
              className="primary-button mt-3 flex w-full items-center justify-center gap-1.5"
            >
          ...
          <select
            ...
            className="field-control mt-3 text-sm font-medium"
          >
      ...
      <section className="mt-6 px-5">
        <h3 className="text-sm font-semibold text-brand-dark">
          <span className="mr-2 font-bold tabular-nums text-brand">02</span> 서류를 올려주세요
        </h3>

        <label className="mt-3 flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-brand-light bg-surface py-10 active:bg-black/[0.02]">
          <input
            type="file"
          ...
          {file ? (
            <>
              ...
              <p className="mt-3 max-w-[240px] truncate px-4 text-sm font-semibold text-brand-dark">
                {file.name}
              </p>
            ...
            <>
              ...
              <p className="mt-3 text-sm font-semibold text-brand-dark/70">파일 선택</p>
              <p className="mt-1 text-xs text-muted">PDF · HWP · DOCX · XLSX</p>
            </>
          )}
        </label>
        ...
        <button
          onClick={onSubmit}
          disabled={!file}
          className="primary-button mt-5 w-full py-3.5 text-base"
        >
          검토 시작하기
        </button>
      </section>
                ...
                <span
                  className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md ${
                      ...
                      : active
                        ...
                        : 'border border-line text-brand-dark/30'
                  }`}
                >
                  {done ? (
...
function Result({ review, onReset }: { review: ReviewResponse; onReset: () => void }) {
  ...
  if (failed || !result) {
    return (
      <section className="mt-8 px-5">
        <div className="surface-panel border-l-4 border-l-status-red p-5">
          ...
          <h3 className="mt-4 text-base font-semibold text-brand-dark">검토하지 못했습니다</h3>
          <p className="mt-2 text-sm leading-relaxed text-muted">
            {result?.overall || '서류를 읽는 데 실패했습니다.'}
          </p>
          <button
            onClick={onReset}
            className="primary-button mt-5 flex items-center justify-center gap-1.5"
          >
  ...
  return (
    <section className="mt-6 space-y-4 px-5">
      {/* 종합 진단 */}
      <div className="surface-panel border-l-4 border-l-brand p-5">
        <p className="text-xs font-semibold text-muted">{result.document_type}</p>
        <p className="mt-2 text-[15px] font-medium leading-relaxed text-brand-dark">
          {result.overall}
        </p>
      </div>
      ...
      <button
        onClick={onReset}
        className="secondary-button flex w-full items-center justify-center gap-1.5"
      >
